...
После применения миграций начальный администратор создаётся отдельной командой. Значения берутся из окружения и не сохраняются в репозитории:
...
Команда идемпотентна: повторный запуск не меняет пароль существующего пользователя, но сохраняет за ним прикладной и технический административный доступ.
